...
import { getServerDiskUsageMB } from "../services/diskUsage.js";
...
export const getServers = async (req: Request, res: Response) => {
  ...
  // Update statuses and disk usage
  const updatedServers = await Promise.all(userServers.map(async (server: any) => {
    const diskMB = await getServerDiskUsageMB(server.id);
    server.diskUsageMB = diskMB;
    server.diskUsageGB = parseFloat((diskMB / 1024).toFixed(2));
...
export const getServerStats = async (req: Request, res: Response) => {
  ...
  if (server.containerId) {
    ...
    res.json({
      ...stats,
      disk: stats?.disk !== undefined ? stats.disk : diskGB,
...
export const createServer = async (req: Request, res: Response) => {
  ...
  const { name, ram, port, version, theme, cpu, disk, owner, ownerId, ipAlias, type, nodeId, runtimeType, javaVersion } = req.body;
  if (!name || !ram || !port) {
...
export const installPlugin = async (req: Request, res: Response) => {
  ...
  try {
    ...
    } else if (source === 'spigot' || source === 'bukkit') {
