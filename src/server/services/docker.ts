...
const ensureTopVersions = (vers: string[]) => {
  const essential = ["latest", "26.2", "26.1.2", "1.21.4", "1.21.3", "1.21.1", "1.21"];
  const set = new Set<string>();
  const out: string[] = [];
  
  // Add essential versions in order
  ...
  // Add remaining versions from dynamic list
  for (const v of vers) {
    if (!set.has(v)) {
      set.add(v);
      out.push(v);
...
export const getVersions = async (type: string = "PAPER"): Promise<string[]> => {
  const normalizedType = (type || "PAPER").toUpperCase();
  ...
  if (normalizedType === "FORGE") {
    return [
      "latest", "26.2", "26.1.2", "1.21.4", "1.21.1", "1.20.4", "1.20.1", "1.19.4", "1.19.2", "1.18.2", "1.16.5", "1.12.2"
    ];
  }
  if (normalizedType === "SPIGOT") {
    return [
      "latest", "26.2", "26.1.2", "1.21.4", "1.21.3", "1.21.1", "1.21", "1.20.6", "1.20.4", "1.20.2", "1.20.1",
      "1.19.4", "1.19.2", "1.18.2", "1.17.1", "1.16.5", "1.15.2", "1.14.4", "1.13.2", "1.12.2", "1.8.8"
    ];
  }
  
  // Check cache for dynamic APIs (Paper, Purpur, Fabric, Vanilla, Folia)
  const cached = versionCache[normalizedType];
  ...
  try {
    if (normalizedType === "PAPER") {
      const res = await axios.get("https://api.papermc.io/v2/projects/paper", { timeout: 4000 });
      if (res.data?.versions && Array.isArray(res.data.versions)) {
        const reversed = [...res.data.versions].reverse();
        resultVersions = ensureTopVersions(reversed);
      }
    } else if (normalizedType === "PURPUR") {
      const res = await axios.get("https://api.purpurmc.org/v2/purpur", { timeout: 4000 });
      if (res.data?.versions && Array.isArray(res.data.versions)) {
        const reversed = [...res.data.versions].reverse();
        resultVersions = ensureTopVersions(reversed);
      }
    } else if (normalizedType === "FOLIA") {
      try {
        const res = await axios.get("https://api.papermc.io/v2/projects/folia", { timeout: 4000 });
        if (res.data?.versions && Array.isArray(res.data.versions)) {
          const reversed = [...res.data.versions].reverse();
          resultVersions = ensureTopVersions(reversed);
        }
      } catch (e) {
        resultVersions = ensureTopVersions(["1.21.4", "1.21.3", "1.21.1", "1.20.6", "1.20.4", "1.20.2", "1.20.1", "1.19.4"]);
      }
    } else if (normalizedType === "FABRIC") {
      const res = await axios.get("https://meta.fabricmc.net/v2/versions/game", { timeout: 4000 });
      if (Array.isArray(res.data)) {
        const releases = res.data.filter((v: any) => v.stable).map((v: any) => v.version);
        resultVersions = ensureTopVersions(releases);
      }
    } else if (normalizedType === "VANILLA") {
      const res = await axios.get("https://piston-meta.mojang.com/mc/game/version_manifest_v2.json", { timeout: 4000 });
      if (res.data?.versions && Array.isArray(res.data.versions)) {
        const releases = res.data.filter((v: any) => v.type === "release").map((v: any) => v.id);
        resultVersions = ensureTopVersions(releases);
      }
    }
  } catch (err) {
  ...
  if (resultVersions.length > 0) {
    versionCache[normalizedType] = { timestamp: Date.now(), versions: resultVersions };
...
export const getContainerStats = async (containerId: string, nodeId?: string, serverId?: string) => {
  const docker = await getDocker(nodeId);
  const targetServerId = serverId || (containerId.startsWith("mock-container-id-") ? containerId.replace("mock-container-id-", "") : undefined);
  const diskMB = targetServerId ? await getServerDiskUsageMB(targetServerId) : 0;
  const diskGB = parseFloat((diskMB / 1024).toFixed(2));

  if (isNodeSandbox(nodeId)) {
    const id = containerId.replace("mock-container-id-", "");
    if (!mockState[id]) return { cpu: 0, ram: 0, disk: diskGB, diskMB };
    ...
    return {
      cpu: parseFloat((floatPseudo * 10 + 2).toFixed(1)), // 2% to 12%
      ram: parseFloat((600 + (floatPseudo * 50 - 25)).toFixed(1)), // ~600 MB
      disk: diskGB,
      diskMB
    };
  }
  try {
    ...
    const info = await container.inspect();
    if (!info.State.Running) {
      return { cpu: 0, ram: 0, disk: diskGB, diskMB };
    }
    ...
    return {
      ...
      ram: parseFloat(ramMB.toFixed(1)),
      disk: diskGB,
      diskMB
    };
  } catch (e) {
